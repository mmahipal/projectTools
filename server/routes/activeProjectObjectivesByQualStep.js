...
router.get('/', authenticate, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.log('[Active Project Objectives by Qual Step] Starting data fetch...');
    ...
    try {
      ...
      console.log('[Active Project Objectives by Qual Step] Using Qualification_Step__c object');
    } catch (error) {
      ...
      try {
        ...
        console.log('[Active Project Objectives by Qual Step] Using Project_Qualification_Step__c object (fallback)');
      } catch (fallbackError) {
        console.error('[Active Project Objectives by Qual Step] Neither Qualification_Step__c nor Project_Qualification_Step__c found');
      }
    }
    ...
    if (OFFSET === 0) {
      try {
        ...
        console.error('[Active Project Objectives by Qual Step] Error getting total count:', error);
      }
    }
    ...
    console.log(`[Active Project Objectives by Qual Step] Found ${qualSteps.length} Qualification Steps (offset: ${OFFSET}, limit: ${LIMIT}, total: ${totalCount}, hasMore: ${hasMore})`);
    
    // Debug: Log sample qualification step
    if (qualSteps.length > 0) {
      console.log(`[Active Project Objectives by Qual Step] Sample Qualification Step:`, {
        Id: qualSteps[0].Id,
    ...
    try {
      ...
      console.log(`[Active Project Objectives by Qual Step] Qualification Step object fields (first 20):`, 
      ...
      for (const fieldName of possibleProjectObjectiveFields) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found Project Objective field on Qualification Step: ${fieldName}`);
          break;
        }
      }
      ...
      if (!projectObjectiveField) {
        ...
        if (referenceFields.length > 0) {
          console.log(`[Active Project Objectives by Qual Step] Found potential Project Objective reference fields:`, 
    ...
    try {
      ...
      console.log(`[Active Project Objectives by Qual Step] Project_Objective__c object fields (first 30):`, 
      ...
      for (const fieldName of possibleQualStepFields) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found relationship field on Project_Objective__c: ${fieldName}`);
          break;
        }
      }
      ...
      if (!qualStepFieldOnProjectObjective) {
        ...
        if (referenceFields.length > 0) {
          console.log(`[Active Project Objectives by Qual Step] Found potential Qualification Step reference fields on Project_Objective__c:`, 
            referenceFields.map(f => f.name)
          );
          // Try the first one
          qualStepFieldOnProjectObjective = referenceFields[0].name;
          console.log(`[Active Project Objectives by Qual Step] Trying field: ${qualStepFieldOnProjectObjective}`);
        } else {
          console.warn('[Active Project Objectives by Qual Step] No Qualification Step field found on Project_Objective__c');
        }
      }
    } catch (error) {
    ...
    try {
      ...
      console.log(`[Active Project Objectives by Qual Step] Found Project_Qualification_Step__c object with ${projectQualStepDescribe.fields.length} fields`);
      ...
      for (const fieldName of possibleQualStepFields) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found Qualification Step field on Project_Qualification_Step__c: ${fieldName} (type: ${field.type}, referenceTo: ${field.referenceTo ? field.referenceTo.join(',') : 'N/A'})`);
          break;
        }
      }
      ...
      if (!projectQualStepQualStepField) {
        ...
        if (referenceFields.length > 0) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found potential Qualification Step reference field on Project_Qualification_Step__c: ${projectQualStepQualStepField}`);
        }
      }
      ...
      for (const fieldName of possibleProjectObjectiveFields) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found Project Objective field on Project_Qualification_Step__c: ${fieldName} (type: ${field.type}, referenceTo: ${field.referenceTo ? field.referenceTo.join(',') : 'N/A'})`);
          break;
        }
      }
      ...
      if (!projectQualStepProjectObjectiveField) {
        ...
        if (referenceFields.length > 0) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found potential Project Objective reference field on Project_Qualification_Step__c: ${projectQualStepProjectObjectiveField}`);
        }
      }
      
      if (projectQualStepQualStepField && projectQualStepProjectObjectiveField) {
        console.log(`[Active Project Objectives by Qual Step] Using Project_Qualification_Step__c junction: ${projectQualStepQualStepField} -> ${projectQualStepProjectObjectiveField}`);
      }
    } catch (error) {
      console.log(`[Active Project Objectives by Qual Step] Project_Qualification_Step__c object not found or error: ${error.message}`);
    }
    ...
    try {
      ...
      console.log(`[Active Project Objectives by Qual Step] Project_Page__c has ${projectPageDescribe.fields.length} fields`);
      ...
      for (const fieldName of possibleQualStepFieldsOnPage) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found Qualification Step field on Project_Page__c: ${fieldName} (type: ${field.type})`);
          break;
        }
      }
      ...
      if (!projectPageQualStepField) {
        ...
        if (referenceFields.length > 0) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found potential Qualification Step reference field: ${projectPageQualStepField}`);
        }
      }
      ...
      for (const fieldName of possibleProjectObjectiveFieldsOnPage) {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found Project Objective field on Project_Page__c: ${fieldName} (type: ${field.type})`);
          break;
        }
      }
      ...
      if (!projectPageProjectObjectiveField) {
        ...
        if (referenceFields.length > 0) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Found potential Project Objective reference field: ${projectPageProjectObjectiveField}`);
        }
      }
      
      if (projectPageQualStepField && projectPageProjectObjectiveField) {
        console.log(`[Active Project Objectives by Qual Step] Found relationship through Project_Page__c: ${projectPageQualStepField} -> ${projectPageProjectObjectiveField}`);
        ...
        try {
          ...
          console.log(`[Active Project Objectives by Qual Step] Project_Page__c records with both fields: ${testResult.totalSize || 0}`);
        } catch (testError) {
          console.error(`[Active Project Objectives by Qual Step] Error testing Project_Page__c query:`, testError.message);
        }
      } else {
        console.warn(`[Active Project Objectives by Qual Step] Project_Page__c relationship not found. QualStepField: ${projectPageQualStepField}, ProjectObjectiveField: ${projectPageProjectObjectiveField}`);
      }
    } catch (error) {
    ...
    if (projectQualStepObjectExists && projectQualStepQualStepField && projectQualStepProjectObjectiveField) {
      ...
      for (let i = 0; i < qualStepIds.length; i += QUAL_STEP_BATCH_SIZE) {
        ...
        try {
          console.log(`[Active Project Objectives by Qual Step] Querying Project_Qualification_Step__c for batch ${batchNumber} (${batch.length} qualification steps)`);
          ...
          console.log(`[Active Project Objectives by Qual Step] Batch ${batchNumber}: Found ${batchProjectQualStepRecords.length} Project_Qualification_Step__c records`);
        } catch (error) {
          console.error(`[Active Project Objectives by Qual Step] Error querying Project_Qualification_Step__c batch ${batchNumber}:`, error.message);
          continue;
        }
      }
      
      console.log(`[Active Project Objectives by Qual Step] Total Project_Qualification_Step__c records found: ${allProjectQualStepRecords.length}`);
      ...
      console.log(`[Active Project Objectives by Qual Step] Unique Project Objective IDs from Project_Qualification_Step__c: ${projectObjectiveIdsFromQualSteps.length}`);
      ...
      if (projectObjectiveIdsFromQualSteps.length > 0) {
        ...
        for (let i = 0; i < projectObjectiveIdsFromQualSteps.length; i += PO_BATCH_SIZE) {
          ...
          try {
            ...
            console.error(`[Active Project Objectives by Qual Step] Error querying Project Objectives batch:`, error.message);
            continue;
          }
        }
      }
      ...
      console.log(`[Active Project Objectives by Qual Step] Built mapping through Project_Qualification_Step__c: ${allProjectQualStepRecords.length} records mapped`);
      console.log(`[Active Project Objectives by Qual Step] Qualification Steps with Project Objectives:`, 
      ...
      for (let i = 0; i < qualStepIds.length; i += QUAL_STEP_BATCH_SIZE) {
        ...
        try {
          console.log(`[Active Project Objectives by Qual Step] Querying Project Objectives for batch ${batchNumber}`);
          let poResult = await conn.query(poQuery);
          let batchProjectObjectives = poResult.records || [];
          ...
          console.error(`[Active Project Objectives by Qual Step] Error querying batch ${batchNumber}:`, error.message);
          continue;
        }
      }
      ...
      for (let i = 0; i < qualStepIds.length; i += QUAL_STEP_BATCH_SIZE) {
        ...
        try {
          console.log(`[Active Project Objectives by Qual Step] Querying Project Objectives for batch ${batchNumber} (reverse relationship)`);
          let poResult = await conn.query(poQuery);
          let batchProjectObjectives = poResult.records || [];
      ...
      console.log(`[Active Project Objectives by Qual Step] Using Project_Page__c junction: ${projectPageQualStepField} -> ${projectPageProjectObjectiveField}`);
      ...
      for (let i = 0; i < qualStepIds.length; i += QUAL_STEP_BATCH_SIZE) {
        ...
        try {
          console.log(`[Active Project Objectives by Qual Step] Querying Project Pages for batch ${batchNumber} (${batch.length} qualification steps)`);
          ...
          console.log(`[Active Project Objectives by Qual Step] Batch ${batchNumber}: Found ${batchProjectPageRecords.length} Project Pages`);
        } catch (error) {
          console.error(`[Active Project Objectives by Qual Step] Error querying Project Pages batch ${batchNumber}:`, error.message);
          continue;
        }
      }
      
      console.log(`[Active Project Objectives by Qual Step] Total Project Pages found: ${allProjectPageRecords.length}`);
      ...
      console.log(`[Active Project Objectives by Qual Step] Unique Project Objective IDs from Project Pages: ${projectObjectiveIdsFromPages.length}`);
      ...
      console.log(`[Active Project Objectives by Qual Step] Built mapping through Project_Page__c: ${allProjectPageRecords.length} Project Pages mapped`);
      console.log(`[Active Project Objectives by Qual Step] Qualification Steps with Project Objectives:`, 
        Array.from(projectObjectivesByQualStep.entries())
          .filter(([id, pos]) => pos.length > 0)
          .length
      ...
      console.warn('[Active Project Objectives by Qual Step] No relationship found between Qualification Steps and Project Objectives');
      console.warn('[Active Project Objectives by Qual Step] Tried: direct field on Qualification Step, reverse field on Project Objective, and Project_Page__c junction');
      console.warn(`[Active Project Objectives by Qual Step] Relationship discovery summary:`, {
    ...
    console.log(`[Active Project Objectives by Qual Step] Found ${projectObjectives.length} Project Objectives`);
    ...
    console.log(`[Active Project Objectives by Qual Step] Project Objectives by Qual Step mapping:`, 
      Array.from(projectObjectivesByQualStep.entries())
        .filter(([id, pos]) => pos.length > 0)
        .slice(0, 5)
        .map(([id, pos]) => ({ qualStepId: id, count: pos.length }))
    );
    console.log(`[Active Project Objectives by Qual Step] Total Qualification Steps with Project Objectives:`, 
    ...
    console.log(`[Active Project Objectives by Qual Step] Processing ${projectObjectiveIds.length} Project Objectives in ${Math.min(totalBatches, MAX_BATCHES)} batches`);
    ...
    try {
      ...
      console.log(`[Active Project Objectives by Qual Step] Searching for qualification step field on Contributor_Project__c...`);
      console.log(`[Active Project Objectives by Qual Step] Total fields on Contributor_Project__c: ${cpDescribeResult.fields.length}`);
      
      // List all reference fields for debugging
      const allReferenceFields = cpDescribeResult.fields.filter(f => f.type === 'reference');
      console.log(`[Active Project Objectives by Qual Step] Found ${allReferenceFields.length} reference fields on Contributor_Project__c`);
      ...
      console.log(`[Active Project Objectives by Qual Step] All reference fields on Contributor_Project__c:`);
      ...
      for (const fieldName of possibleQualStepFieldsOnCP) {
        ...
        if (field) {
          console.log(`[Active Project Objectives by Qual Step] Found field ${fieldName}, type: ${field.type}, referenceTo: ${(field.referenceTo || []).join(', ')}`);
          if (field.type === 'reference') {
            ...
            if (referenceTo.includes('Qualification_Step__c') || 
              ...
              console.log(`[Active Project Objectives by Qual Step] ✓ Found qualification step field on Contributor_Project__c: ${fieldName} (references: ${referenceTo.join(', ')})`);
              break;
            }
            ...
            console.log(`[Active Project Objectives by Qual Step] Found ${field.type} field ${fieldName} - might store qualification step info`);
          }
        }
      }
      ...
      if (!qualStepFieldOnCP) {
        ...
        console.log(`[Active Project Objectives by Qual Step] Found ${referenceFields.length} reference fields with 'Qualification', 'Qual', or 'Step' in name`);
        if (referenceFields.length > 0) {
          // Check if any of these reference Qualification Step objects
          for (const field of referenceFields) {
            const referenceTo = field.referenceTo || [];
            console.log(`[Active Project Objectives by Qual Step] Checking field ${field.name}, referenceTo: ${referenceTo.join(', ')}`);
            if (referenceTo.some(ref => ref.includes('Qualification') && ref.includes('Step'))) {
              qualStepFieldOnCP = field.name;
              console.log(`[Active Project Objectives by Qual Step] ✓ Found potential qualification step field: ${field.name} (references: ${referenceTo.join(', ')})`);
              break;
            }
          }
        }
      ...
      if (!qualStepFieldOnCP) {
        console.log(`[Active Project Objectives by Qual Step] Checking ALL reference fields for Qualification Step references...`);
        for (const field of allReferenceFields) {
          ...
          if (referenceTo.some(ref => 
            ...
            qualStepFieldOnCP = field.name;
            console.log(`[Active Project Objectives by Qual Step] ✓ Found qualification step field by checking all references: ${field.name} (references: ${referenceTo.join(', ')})`);
            break;
          }
        }
      }
      ...
      if (!qualStepFieldOnCP) {
        console.warn(`[Active Project Objectives by Qual Step] ⚠ No qualification step field found on Contributor_Project__c. Will count all active contributors for project objectives.`);
        console.warn(`[Active Project Objectives by Qual Step] Available reference fields with 'Qual' or 'Step' in name:`, 
          allReferenceFields
        ...
        console.log(`[Active Project Objectives by Qual Step] Checking for relationship to Project_Qualification_Step__c...`);
        ...
        if (projectQualStepRefFields.length > 0) {
          console.log(`[Active Project Objectives by Qual Step] Found ${projectQualStepRefFields.length} field(s) referencing Project_Qualification_Step__c:`, 
            projectQualStepRefFields.map(f => f.name).join(', '));
          // Use the first one found
          qualStepFieldOnCP = projectQualStepRefFields[0].name;
          console.log(`[Active Project Objectives by Qual Step] ✓ Will use ${qualStepFieldOnCP} to filter by Project_Qualification_Step__c`);
        } else {
          // Check if there's a junction object like Contributor_Qualification_Step__c
          console.log(`[Active Project Objectives by Qual Step] Checking for junction objects that might link Contributor_Project__c to Qualification Steps...`);
          try {
            ...
            for (const junctionObjectName of possibleJunctionObjects) {
              try {
                ...
                console.log(`[Active Project Objectives by Qual Step] Found junction object: ${junctionObjectName}`);
                ...
                if (cpRefFields.length > 0 && qualStepRefFields.length > 0) {
                  console.log(`[Active Project Objectives by Qual Step] Found junction object ${junctionObjectName} with fields:`, {
                  ...
                  console.log(`[Active Project Objectives by Qual Step] Will use junction object ${junctionObjectName} to filter contributors by qualification step`);
                  break;
                }
              } catch (error) {
            ...
            console.error(`[Active Project Objectives by Qual Step] Error checking for junction objects:`, error.message);
          }
        }
      } else {
        console.log(`[Active Project Objectives by Qual Step] ✓ Using qualification step field: ${qualStepFieldOnCP}`);
      }
    } catch (error) {
      console.error('[Active Project Objectives by Qual Step] Error describing Contributor_Project__c for qualification step field:', error);
    }
    ...
    console.log(`[Active Project Objectives by Qual Step] Completed in ${executionTime}ms`);
    console.log(`[Active Project Objectives by Qual Step] Returning ${result.length} qualification steps with contributors`);
    if (result.length > 0) {
      console.log(`[Active Project Objectives by Qual Step] Sample result:`, {
    ...
    const executionTime = Date.now() - startTime;
    console.error(`[Active Project Objectives by Qual Step] Error after ${executionTime}ms:`, error);
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    try {
      ...
      if (projectQualStepQualStepField && projectQualStepProjectObjectiveField) {
        ...
        try {
          ...
          console.error('[Active Project Objectives by Qual Step] Error getting Qualification Step name:', error);
        }
        ...
        console.log(`[Active Project Objectives by Qual Step] Found ${projectObjectiveIds.length} Project Objectives via Project_Qualification_Step__c for Qualification Step ${qualStepId}`);
      }
    } catch (error) {
      console.log(`[Active Project Objectives by Qual Step] Project_Qualification_Step__c not available, trying other methods: ${error.message}`);
    }
    ...
    for (const fieldName of possibleContributorFields) {
      try {
        ...
        console.log(`[Active Project Objectives by Qual Step] Found contributor field: ${contributorField}`);
        break;
      } catch (error) {
        continue;
      }
    ...
    if (!contributorField) {
      ...
      try {
        ...
        if (field) {
          ...
          console.log(`[Active Project Objectives by Qual Step] Discovered contributor field: ${contributorField}`);
        }
      } catch (error) {
        console.error('Error describing Contributor_Project__c:', error);
      }
    ...
    try {
      ...
      for (const fieldName of possibleQualStepFieldsOnCP) {
        ...
        if (field && field.type === 'reference') {
          ...
          if (referenceTo.includes('Qualification_Step__c') || 
              ...
              referenceTo.some(ref => ref.includes('Qualification') && ref.includes('Step'))) {
            qualStepFieldOnCP = fieldName;
            console.log(`[Active Project Objectives by Qual Step] Found qualification step field on Contributor_Project__c: ${fieldName}`);
            break;
          }
        }
      }
      ...
      if (!qualStepFieldOnCP) {
        ...
        if (referenceFields.length > 0) {
          for (const field of referenceFields) {
            ...
            if (referenceTo.some(ref => ref.includes('Qualification') && ref.includes('Step'))) {
              qualStepFieldOnCP = field.name;
              console.log(`[Active Project Objectives by Qual Step] Found potential qualification step field: ${field.name}`);
              break;
            }
          }
        }
      ...
      if (!qualStepFieldOnCP) {
        console.warn(`[Active Project Objectives by Qual Step] No qualification step field found on Contributor_Project__c. Will return all active contributors for project objectives.`);
        console.warn(`[Active Project Objectives by Qual Step] All reference fields on Contributor_Project__c:`, 
    ...
    for (let i = 0; i < projectObjectiveIds.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.log(`[Active Project Objectives by Qual Step] Batch ${batchNumber}/${Math.ceil(projectObjectiveIds.length / BATCH_SIZE)}: Fetched ${batchRecords.length} Contributor Project records (Total so far: ${contributorProjectRecords.length})`);
      } catch (error) {
        console.error(`[Active Project Objectives by Qual Step] Error querying Contributor Projects batch ${batchNumber}:`, error.message);
        continue;
      }
    }
    
    console.log(`[Active Project Objectives by Qual Step] Total Contributor Project records: ${contributorProjectRecords.length}`);
    ...
    for (let i = 0; i < projectObjectiveIds.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        console.error(`[Active Project Objectives by Qual Step] Error querying Project Objective names batch:`, error.message);
        continue;
      }
    }
    ...
    console.log(`[Active Project Objectives by Qual Step] Found ${contributorIds.length} unique contributor IDs from ${contributorProjectRecords.length} Contributor Project records`);
    ...
    for (let i = 0; i < contributorIds.length; i += CONTACT_BATCH_SIZE) {
      ...
      try {
        ...
        console.error('[Active Project Objectives by Qual Step] Error describing Contact object:', describeError);
      ...
      try {
        ...
        console.error(`[Active Project Objectives by Qual Step] Error querying contacts batch:`, error.message);
        ...
        try {
          ...
          console.error(`[Active Project Objectives by Qual Step] Error querying basic contact fields:`, basicError.message);
        }
      }
    }
    ...
    console.error('[Active Project Objectives by Qual Step] Error fetching contributor names:', error);
    res.status(500).json({
      success: false,
