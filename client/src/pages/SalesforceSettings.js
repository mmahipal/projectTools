...
const SalesforceSettings = ({ asTab = false }) => {
  ...
  // Use ref to track if test is in progress (prevents multiple concurrent requests)
  const testInProgressRef = useRef(false);
  const lastTestTimeRef = useRef(0);
  const DEBOUNCE_DELAY_MS = 2000; // 2 seconds debounce between test requests
  ...
  const testConnection = useCallback(async () => {
    ...
    if (timeSinceLastTest < DEBOUNCE_DELAY_MS) {
      const remainingTime = Math.ceil((DEBOUNCE_DELAY_MS - timeSinceLastTest) / 1000);
      toast.error(`Please wait ${remainingTime} second(s) before testing again`);
      return;
    }
  ...
  const testProjectCreation = async () => {
    ...
    try {
      ...
      // If it's a connection error, use the user-friendly message from the interceptor
      if (error.isBackendDown || error.userMessage) {
        errorMessage = error.userMessage || error.message;
      }
      
      // Fallback to generic message if no specific error
      if (!errorMessage || errorMessage.includes('port 5000')) {
        errorMessage = error.response?.status === 504 
          ? 'Request timed out. The server may be processing a large request. Please try again.'
          : 'Project creation API test failed. Please check your network connection and try again.';
      }
      
      setProjectCreationTestStatus({
        success: false,
        message: errorMessage,
        details: error.response?.data?.details || (error.response?.data ? JSON.stringify(error.response.data, null, 2) : undefined)
      });
      toast.error(errorMessage);
      console.error('Salesforce project creation test error:', {
        status: error.response?.status,
        data: error.response?.data,
        message: error.message,
        code: error.code,
        isBackendDown: error.isBackendDown
      });
    } finally {
