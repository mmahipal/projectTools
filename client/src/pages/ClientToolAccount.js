...
const ClientToolAccount = () => {
  ...
  const [conflictConfirmData, setConflictConfirmData] = useState(null);
  const [showInactiveConfirm, setShowInactiveConfirm] = useState(false);
  const [inactiveConfirmData, setInactiveConfirmData] = useState(null);
  ...
  // Define fetchClientToolNamePicklist BEFORE it's used in useEffect hooks
  const fetchClientToolNamePicklist = useCallback(async () => {
    try {
  ...
  // Define fetchContributorProjects BEFORE it's used in useEffect hooks
  ...
  // Load management tab data on initial mount (lazy loading)
  useEffect(() => {
    if (activeTab === 'management' && !loadedTabs.has('management')) {
      fetchContributorProjects();
      fetchClientToolNamePicklist();
      setLoadedTabs(prev => new Set([...prev, 'management']));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // Only run on initial mount
  
  // Load management tab data when tab is clicked (lazy loading)
  useEffect(() => {
    if (activeTab === 'management' && !loadedTabs.has('management')) {
      fetchContributorProjects();
      fetchClientToolNamePicklist();
  ...
  }, [activeTab, loadedTabs, fetchContributorProjects, fetchClientToolNamePicklist]);
  ...
  const handleExportToExcel = () => {
    ...
    try {
      // Prepare data for export - match table columns exactly
      const exportData = filteredContributorProjects.map(project => ({
        ...
        'Project Objective': project.projectObjective || 'N/A',
        'Client Tool Account Used': project.clientToolAccountUsedName || project.clientToolAccountUsed || 'Not Set'
      }));
  ...
  const handleImportFile = async (event) => {
    ...
    const processImportData = async (jsonData) => {
      ...
      try {
        ...
        // Use apiClient with extended timeout for bulk import
        // apiClient already includes CSRF token handling via interceptors
        const response = await apiClient.post('/client-tool-account/bulk-import', {
          records: jsonData,
  ...
  const handlePublishSelected = async () => {
    ...
    if (relevantConflicts.length > 0) {
      ...
      setConflictConfirmData({ conflictMessages, continueCallback: () => {
  ...
  const checkInactiveAccounts = () => {
    ...
    if (inactiveAccounts.length > 0) {
      const inactiveMessage = inactiveAccounts
        ...
        .join('\n');
      setInactiveConfirmData({ inactiveMessage, continueCallback: () => {
        setShowInactiveConfirm(false);
        proceedWithBulkUpdate();
      }});
  ...
  return (
    <div className="dashboard-layout">
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                    ...
                    <div className="form-group" style={{ position: 'relative', marginBottom: '10px' }}>
                      ...
                      {!formData.contributor && !formData.contributorId && contributorSearchTerm && contributorSearchTerm.trim().length > 0 && contributorSearchResults.length === 0 && !contributorSearching && (
                    ...
                    <div className="form-group" style={{ position: 'relative', marginBottom: '10px' }}>
                      ...
                      {!formData.account && !formData.accountId && accountSearchTerm && accountSearchTerm.trim().length > 0 && accountSearchResults.length === 0 && !accountSearching && (
                        <div style={{ color: 'red', fontSize: '11px', marginTop: '2px' }}>Complete this field.</div>
                      )}
                    </div>
                  </div>
      ...
      <ConfirmModal
        ...
        message={conflictConfirmData ? `Warning: Conflicts detected:\n\n${conflictConfirmData.conflictMessages}\n\nDo you want to proceed anyway?` : ''}
        onConfirm={() => {
          if (conflictConfirmData?.continueCallback) {
            conflictConfirmData.continueCallback();
          }
        }}
        onCancel={() => {
          setShowConflictConfirm(false);
      ...
      <ConfirmModal
        ...
        message={inactiveConfirmData ? `Warning: Some accounts are inactive or deactivated:\n\n${inactiveConfirmData.inactiveMessage}\n\nDo you want to proceed anyway?` : ''}
        onConfirm={() => {
          if (inactiveConfirmData?.continueCallback) {
            inactiveConfirmData.continueCallback();
          }
        }}
        onCancel={() => {
          setShowInactiveConfirm(false);
          setInactiveConfirmData(null);
        }}
        confirmText="Yes, Proceed"
        cancelText="Cancel"
        type="warning"
