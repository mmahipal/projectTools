...
import { Search, Info, Menu, Send, Save, BookOpen, CheckCircle, XCircle, X } from 'lucide-react';
...
const ProjectQualificationStepSetup = () => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // Publishing qualification step
        totalFields: Object.keys(cleanedData).length,
        fields: Object.keys(cleanedData)
      });

      // Publishing qualification step to Salesforce
        totalFields: Object.keys(cleanedData).length,
        fields: Object.keys(cleanedData),
        sampleData: {
      ...
      if (response.data.success) {
        ...
        // Salesforce qualification step created
