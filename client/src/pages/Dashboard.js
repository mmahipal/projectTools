...
const Dashboard = () => {
  ...
  const fetchStats = useCallback(async (silent = false) => {
    try {
      ...
      // Use stale-while-revalidate for stats (returns cached immediately, refreshes in background)
      const response = await staleWhileRevalidate(
        '/projects/stats',
        {},
        async () => apiClient.get('/projects/stats'),
        { ttl: 60 * 1000, staleThreshold: 0.8 }
      );
  ...
  // Set up visibility-aware polling - refresh every 60 seconds (increased from 10s)
  // Automatically pauses when tab is hidden and resumes when visible
  useVisibilityAwarePolling(
    () => {
      fetchStats(true); // Silent refresh
    },
    60000, // 60 seconds (increased from 10 seconds)
    {
      enabled: true,
