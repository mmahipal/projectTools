...
const UserManagement = ({ asTab = false }) => {
  ...
  const [userSettings, setUserSettings] = useState({}); // Store user settings by userId
  ...
  const fetchUsers = async () => {
    ...
    try {
      const [usersResponse, settingsResponse] = await Promise.all([
        ...
        apiClient.get('/auth/users/settings').catch(() => ({ data: { success: false, userSettings: [] } })) // Don't fail if settings endpoint fails
      ]);
      ...
      if (settingsResponse.data.success && settingsResponse.data.userSettings) {
        // Convert array to object keyed by userId for easy lookup
        const settingsMap = {};
        settingsResponse.data.userSettings.forEach(us => {
          settingsMap[us.userId] = us;
        });
  ...
  const handleDeleteUser = async (userId) => {
    setDeleteConfirmData({ userId });
    setShowDeleteConfirm(true);
  };

  const confirmDeleteUser = async () => {
    if (!deleteConfirmData) return;
    const { userId } = deleteConfirmData;
