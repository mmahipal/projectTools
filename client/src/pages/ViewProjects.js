...
const ViewProjects = () => {
  ...
  const [retryConfirmData, setRetryConfirmData] = useState(null);
  const [showQuickSetupDeleteConfirm, setShowQuickSetupDeleteConfirm] = useState(false);
  ...
  useEffect(() => {
    ...
    // Load first tab (projects) on initial mount only
    if (isInitialMountRef.current && !loadedTabs.has('projects')) {
      fetchProjects();
      fetchQuickSetup();
      setLoadedTabs(prev => new Set([...prev, 'projects']));
      isInitialMountRef.current = false;
    }
  }, [searchParams, loadedTabs]);
  
  // Load tab data when tab is clicked (lazy loading) - skip on initial mount
  useEffect(() => {
    // Skip on initial mount - initial load is handled by the searchParams effect
    if (isInitialMountRef.current) {
      return;
    }
    
    if (activeTab === 'projects' && !loadedTabs.has('projects')) {
      fetchProjects();
      fetchQuickSetup();
      setLoadedTabs(prev => new Set([...prev, 'projects']));
    } else if (activeTab === 'project-objectives' && !loadedTabs.has('project-objectives')) {
      fetchProjectObjectives();
      setLoadedTabs(prev => new Set([...prev, 'project-objectives']));
    }
  ...
  const handleDelete = async (id, type = 'project') => {
    ...
    setDeleteConfirmData({ id, type, itemType });
    setShowDeleteConfirm(true);
  };

  const confirmDelete = async () => {
    if (!deleteConfirmData) return;
    const { id, type, itemType } = deleteConfirmData;
  ...
  const handlePublishObjective = async (objectiveId) => {
    setPublishConfirmData({ objectiveId });
    setShowPublishConfirm(true);
  };

  const confirmPublishObjective = async () => {
    if (!publishConfirmData) return;
    const { objectiveId } = publishConfirmData;
  ...
  const handleResync = async (projectId) => {
    setRetryConfirmData({ projectId });
    setShowRetryConfirm(true);
  };

  const confirmRetrySync = async () => {
    if (!retryConfirmData) return;
    const { projectId } = retryConfirmData;
    ...
    try {
      ...
      // Resyncing project to Salesforce
        projectId: projectId,
      ...
      setResyncing(prev => ({ ...prev, [projectId]: false }));
      setRetryConfirmData(null);
    }
  };

  const confirmDeleteQuickSetup = async () => {
    setShowQuickSetupDeleteConfirm(false);
    try {
      await deleteDraftQuickSetup();
      setQuickSetupData(null);
      toast.success('Quick setup deleted successfully');
    } catch (error) {
      toast.error('Failed to delete quick setup');
    }
  };
  ...
  return (
    <div className="dashboard-layout">
            ...
            {!quickSetupData ? (
              ...
              <div className="project-card" style={{ maxWidth: '800px', margin: '0 auto' }}>
                ...
                <div className="project-card-actions">
                  ...
                  <button
                    onClick={() => {
                      setShowQuickSetupDeleteConfirm(true);
                    }}
      ...
      <ConfirmModal
        ...
        message={deleteConfirmData ? `Are you sure you want to delete this ${deleteConfirmData.itemType}?` : ''}
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to publish this project objective to Salesforce?"
        onConfirm={confirmPublishObjective}
        onCancel={() => {
          setShowPublishConfirm(false);
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to retry syncing this project to Salesforce?"
        onConfirm={confirmRetrySync}
        onCancel={() => {
          setShowRetryConfirm(false);
          setRetryConfirmData(null);
        }}
      ...
      <ConfirmModal
        show={showQuickSetupDeleteConfirm}
        message="Are you sure you want to delete this quick setup?"
        onConfirm={confirmDeleteQuickSetup}
        onCancel={() => setShowQuickSetupDeleteConfirm(false)}
        confirmText="Yes, Delete"
        cancelText="Cancel"
        type="danger"
      />
