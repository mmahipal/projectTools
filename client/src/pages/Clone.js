...
const Clone = () => {
  ...
  // Get the current name field value based on object type for display in title
  const getCurrentNameValue = () => {
    if (!selectedObjectType || !selectedObject) return null;
    
    // Define name fields for each object type
    const nameFieldMap = {
      'project': 'projectName',
      'project-objective': 'contributorFacingProjectName',
      'project-qualification-step': 'qualificationStep',
      'project-page': 'name',
      'project-team': 'name',
      'contributor-project': 'contributorProjectName'
    };
    
    const nameField = nameFieldMap[selectedObjectType] || 'name';
    const currentValue = watch(nameField);
    const originalName = selectedObject?.name || '';
    
    // If user has entered/changed the name field
    if (currentValue && currentValue.trim() !== '') {
      // Remove "Clone of" prefix if present to get the actual name
      const nameWithoutPrefix = currentValue.startsWith('Clone of ') 
        ? currentValue.replace(/^Clone of /, '') 
        : currentValue;
      
      // If the name (without prefix) is different from original, user changed it
      // Show the current value as-is (user may have kept or removed "Clone of")
      if (nameWithoutPrefix !== originalName || currentValue !== `Clone of ${originalName}`) {
        return currentValue;
      }
    }
    
    // Default: show "Clone of {originalName}" if user hasn't changed it
    return `Clone of ${originalName}`;
  };
          ...
          {step === 'edit' && objectData && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                ...
                <h2 style={{ fontSize: '18px', fontWeight: 600 }}>
                  Create {getObjectTypeLabel(selectedObjectType)}: {getCurrentNameValue() || `Clone of ${selectedObject?.name || ''}`}
                </h2>
              </div>
