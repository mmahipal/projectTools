...
const CaseAnalyticsDashboard = () => {
  ...
  // Filter state with debouncing to prevent excessive API calls
  const [filters, setFilters, debouncedFilters] = useDebouncedFilters({
    date: 'all',
    caseStatus: 'all',
    projectName: 'all',
    accountName: 'all',
  ...
  const fetchAllData = useCallback(async (filtersToUse = null, timeRangeToUse = null) => {
          // Use debouncedFilters by default to prevent excessive API calls
          // But allow override for immediate refreshes (e.g., manual refresh button)
          const activeFilters = filtersToUse || debouncedFilters;
    const activeTimeRange = timeRangeToUse || timeRange;
    setLoading(true);
    try {
      // Fetch KPIs with stale-while-revalidate pattern (returns cached immediately, refreshes in background)
      const kpiResponse = await staleWhileRevalidate(
        '/case-analytics/kpis',
        activeFilters,
        async () => apiClient.get('/case-analytics/kpis', { 
  ...
  }, [activeTab, debouncedFilters, timeRange]);

  // Track which tabs have been loaded
  const [loadedTabs, setLoadedTabs] = useState(new Set());
  
  // Load first tab (daily-snapshot) on initial mount
  useEffect(() => {
  ...
  // Enhanced lazy loading - only load data when tab is actually viewed
  useEffect(() => {
    if (activeTab && !loadedTabs.has(activeTab)) {
      ...
      fetchAllData().catch(error => {
        console.error(`[CaseAnalytics] Error loading tab ${activeTab}:`, error);
  ...
  const handleTimeRangeChange = useCallback((newTimeRange) => {
    ...
    fetchAllData(debouncedFilters, newTimeRange);
  }, [fetchAllData, debouncedFilters]);
  ...
  }, [debouncedFilters, timeRange, saveWidgetToBaseline]);
